...
/**
 * Нэг богино дууг хоцрогдолгүй шууд тоглуулна (тоглоомын товчны хариу дуу).
 * playMorseSequence нь 0.1 сек хойшлуулж эхэлдэг тул товч бүрд хэрэглэхэд
 * хариу удаан мэдрэгддэг.
 */
export function playTone(durationMs, frequency = 600) {
  const ctx = getAudioContext();
  if (!ctx) return;
  if (ctx.state === "suspended") ctx.resume();
  beep(ctx, ctx.currentTime + 0.005, durationMs, frequency);
}
